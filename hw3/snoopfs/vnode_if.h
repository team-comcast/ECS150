...
if (rc == 0) {
	ASSERT_VI_UNLOCKED(dvp, "VOP_CACHEDLOOKUP");
	if (vpp != NULL) {
	ASSERT_VI_UNLOCKED(*vpp, "VOP_CACHEDLOOKUP");
	ASSERT_VOP_LOCKED(*vpp, "VOP_CACHEDLOOKUP");
	}
} else {
	ASSERT_VI_UNLOCKED(dvp, "VOP_CACHEDLOOKUP");
}
...
if (rc == 0) {
	...
	ASSERT_VOP_LOCKED(dvp, "VOP_CREATE");
	if (vpp != NULL) {
	ASSERT_VI_UNLOCKED(*vpp, "VOP_CREATE");
	ASSERT_VOP_LOCKED(*vpp, "VOP_CREATE");
	}
} else {
	ASSERT_VI_UNLOCKED(dvp, "VOP_CREATE");
	ASSERT_VOP_LOCKED(dvp, "VOP_CREATE");
...
if (rc == 0) {
	...
	ASSERT_VOP_LOCKED(dvp, "VOP_MKNOD");
	if (vpp != NULL) {
	ASSERT_VI_UNLOCKED(*vpp, "VOP_MKNOD");
	ASSERT_VOP_LOCKED(*vpp, "VOP_MKNOD");
	}
} else {
	ASSERT_VI_UNLOCKED(dvp, "VOP_MKNOD");
	ASSERT_VOP_LOCKED(dvp, "VOP_MKNOD");
	...
	int fdidx)
{
	...
	CTR5(KTR_VOP, "VOP_OPEN(vp 0x%lX, mode %ld, cred 0x%lX, td 0x%lX, fdidx %ld)", vp, mode, cred, td, fdidx);
if (rc == 0) {
	ASSERT_VI_UNLOCKED(vp, "VOP_OPEN");
	ASSERT_VOP_LOCKED(vp, "VOP_OPEN");
} else {
...
if (rc == 0) {
	...
	ASSERT_VOP_LOCKED(dvp, "VOP_MKDIR");
	if (vpp != NULL) {
	ASSERT_VI_UNLOCKED(*vpp, "VOP_MKDIR");
	ASSERT_VOP_LOCKED(*vpp, "VOP_MKDIR");
	}
} else {
	ASSERT_VI_UNLOCKED(dvp, "VOP_MKDIR");
	ASSERT_VOP_LOCKED(dvp, "VOP_MKDIR");
...
if (rc == 0) {
	...
	ASSERT_VOP_LOCKED(dvp, "VOP_SYMLINK");
	if (vpp != NULL) {
	ASSERT_VI_UNLOCKED(*vpp, "VOP_SYMLINK");
	ASSERT_VOP_LOCKED(*vpp, "VOP_SYMLINK");
	}
} else {
	ASSERT_VI_UNLOCKED(dvp, "VOP_SYMLINK");
	ASSERT_VOP_LOCKED(dvp, "VOP_SYMLINK");
...
if (rc == 0) {
	...
	ASSERT_VOP_LOCKED(vp, "VOP_BMAP");
	if (vpp != NULL) {
	ASSERT_VI_UNLOCKED(*vpp, "VOP_BMAP");
	ASSERT_VOP_UNLOCKED(*vpp, "VOP_BMAP");
	}
} else {
	ASSERT_VI_UNLOCKED(vp, "VOP_BMAP");
	ASSERT_VOP_LOCKED(vp, "VOP_BMAP");
...
extern struct vnodeop_desc vop_specstrategy_desc;
	...
	struct buf *bp)
{
	...
	int rc;
	a.a_desc = VDESC(vop_specstrategy);
	a.a_vp = vp;
	a.a_bp = bp;
#ifdef	DEBUG_VFS_LOCKS
	vop_strategy_pre(&a);
#endif
	rc = VCALL(vp, VOFFSET(vop_specstrategy), &a);
	CTR2(KTR_VOP, "VOP_SPECSTRATEGY(vp 0x%lX, bp 0x%lX)", vp, bp);
if (rc == 0) {
} else {
}
	return (rc);
	...
	struct thread *td)
{
	...
	a.a_cred = cred;
	a.a_td = td;
	ASSERT_VI_UNLOCKED(vp, "VOP_LISTEXTATTR");
	ASSERT_VOP_LOCKED(vp, "VOP_LISTEXTATTR");
	rc = VCALL(vp, VOFFSET(vop_listextattr), &a);
	CTR6(KTR_VOP, "VOP_LISTEXTATTR(vp 0x%lX, attrnamespace %ld, uio 0x%lX, size 0x%lX, cred 0x%lX, td 0x%lX)", vp, attrnamespace, uio, size, cred, td);
if (rc == 0) {
	ASSERT_VI_UNLOCKED(vp, "VOP_LISTEXTATTR");
	ASSERT_VOP_LOCKED(vp, "VOP_LISTEXTATTR");
} else {
	...
	struct thread *td)
{
	...
	a.a_cred = cred;
	a.a_td = td;
	ASSERT_VI_UNLOCKED(vp, "VOP_DELETEEXTATTR");
	ASSERT_VOP_LOCKED(vp, "VOP_DELETEEXTATTR");
	rc = VCALL(vp, VOFFSET(vop_deleteextattr), &a);
	CTR5(KTR_VOP, "VOP_DELETEEXTATTR(vp 0x%lX, attrnamespace %ld, name 0x%lX, cred 0x%lX, td 0x%lX)", vp, attrnamespace, name, cred, td);
if (rc == 0) {
	ASSERT_VI_UNLOCKED(vp, "VOP_DELETEEXTATTR");
	ASSERT_VOP_LOCKED(vp, "VOP_DELETEEXTATTR");
} else {
