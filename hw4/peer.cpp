...
int btPeer::RequestPiece()
{
  ...
  // hw4
  // get the btDequeue system call number and use it to call
  int syscall_num;
  ...
  // if we didnt find it, fail
  if (syscall_num == -1) {
    CONSOLE.Debug("cannot load syscall %s\n", syscall_num);
  } else {
    // dequeue the right number
    btdequeue = syscall(syscall_num);
    printf("btDequeue: %d\n", btdequeue);
  }
